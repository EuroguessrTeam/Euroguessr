...
import { setCurrentGamemode, setListeningTime, setSearchInput, setSkipButtonCounter, useGlobalState } from "../../services/useGlobalState";
    ...
    <>
      ...
      <div className="z-10 relative h-[44.87vh] flex mb-[4vh] mt-[6.5vh] overflow-hidden">
        ...
        <div className="w-[66.68%] h-full flex flex-col justify-between items-center">
          ...
          {currentGamemode.key === GameModeKeys.DAILY &&
            <div className={`opacity-0 ${win ? "opacity-100" : ""} h-[4vh] flex items-center justify-center flex-col text-pretty text-sm`}>
              <p className="text-center  bg-purple rounded-lg p-[1vh]">To see all your scores <b><u>{win && <Link to="/account" className="text-white hover:text-white">click here</Link>}</u></b></p>
              <p className="text-center bg-purple rounded-b-lg px-[1vh] pb-[1vh]">To continue playing <b><u>{win && <button className="underline" onClick={() => switchGameMode(false)}>click here</button>}</u></b></p> 
            </div>
          }
